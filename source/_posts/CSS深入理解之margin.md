...
![](https://merrier.wang/wp-content/uploads/2017/04/一侧定宽的自适应布局.png)

*   滚动容器内上下留白

外部容器设置padding值,只有chrome才会有留白,此时可以在给内部容器设置margin值

![](https://merrier.wang/wp-content/uploads/2017/04/滚动容器内上下留白.png)
...
由于margin百分比的计算规则是相对于容器进行计算的,而margin又可以影响普通block水平元素的可视尺寸,所以当设置为margin:50%时,由于margin重叠特性,所以会让元素的宽高始终保持2:1的比例 ![](https://merrier.wang/wp-content/uploads/2017/04/自适应矩形.png)
...
![](https://merrier.wang/wp-content/uploads/2017/04/相邻的兄弟元素.png)

### 父级和第一个/最后一个子元素

下面这3种书写形式的效果是相同的，都是子元素向下偏移了80px,而父级的尺寸并没有改变 ![](https://merrier.wang/wp-content/uploads/2017/04/父级和子元素.png)
...
![](https://merrier.wang/wp-content/uploads/2017/04/空的block元素.png)
...
![](https://merrier.wang/wp-content/uploads/2017/04/善用margin重叠.png)
...
margin可以改变元素尺寸 ![](https://merrier.wang/wp-content/uploads/2017/04/margin可以改变元素尺寸.png)
...
![](https://merrier.wang/wp-content/uploads/2017/04/margin负值下的等高布局.png)

margin负值下的两栏自适应布局
-----------------

元素占据空间跟随margin移动 ![](https://merrier.wang/wp-content/uploads/2017/04/margin负值下的两栏自适应布局.png)
...
![](https://merrier.wang/wp-content/uploads/2017/04/inline水平元素的margin.png) ![](https://merrier.wang/wp-content/uploads/2017/04/例外的替换元素们.png)
...
内联元素默认基线对齐(vertical-align:baseline),当margin-top是一个很大的负值的时候,由于字母x(内敛元素)的拖累(x没有添加任何样式,所以不会跑到父容器外面),内联元素仍然在与x基线对齐的位置 ![](https://merrier.wang/wp-content/uploads/2017/04/内联特性导致的margin无效.png)
