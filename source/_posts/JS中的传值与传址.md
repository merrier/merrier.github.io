...
在 JS 中，有两种不同的方式可以操作数据的值，这两种技术分别叫做**传值**和**传址**。
...
前面看了这么多类型，一直都没有介绍字符串类型，它不好直接分为基础类型和引用类型，因为字符串可以是任意的长度，看上去应该是引用类型，可是他却不是对象，所以也不可作为引用类型；总结一下，**字符串是按引用(即地址)复制和传递的，但是是按值来比较的。**
...
* [JS 基础知识2 传值和传址](http://www.cnblogs.com/joe2014/archive/2014/06/28/3813919.html)
* [图解JS的传值与传址](http://www.cnblogs.com/idayln/p/3346242.html)
* [JS 传值与传址](http://lxneliu.iteye.com/blog/1567205)