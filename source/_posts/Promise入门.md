...
`Promise.all(\[p1, p2, p3, ...\])` 用于将多个 Promise 实例包装成一个新的 Promise 实例 当所有子 Promise 都完成，该 Promise 完成，返回值是**全部值的数组**；有任何一个失败，该 Promise 失败，返回值是**第一个失败的子Promise的结果**，最常见的是和 .map() 连用
